...
// ==========================
// 🔥 PREDICTIVE INTELLIGENCE (FIXED)
// ==========================

function predictSurplus(donations) {
  ...
  donations.forEach((d) => {
    ...
    timeFrequency[timeLabel] = (timeFrequency[timeLabel] || 0) + 1;
  });

  let predictedTime = null;
  let max = 0;

  for (let time in timeFrequency) {
    if (timeFrequency[time] > max) {
      max = timeFrequency[time];
      predictedTime = time;
    }
  }
...
app.get("/predict", async (_req, res) => {
  try {
    ...
    try {
      ...
      console.log("⚠️ Firebase not configured, using fallback");
    }
    ...
    res.json({
      ...
      predictedTime: predictedTime || "22:00"
    });
